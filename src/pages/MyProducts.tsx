...
function MyProducts() {
  const { purchasedProducts, balance, collectEarnings, getTotalDailyIncome, fetchMyProducts, fetchProfile } = useUserStore()
  ...
  const handleCollect = async () => {
    ...
    if (result.success) {
      showToast(`შემოსავალი დაგერიცხათ! +₾${(result.earned || 0).toFixed(2)}`, 'success')
      fetchMyProducts()
      fetchProfile()
    } else {
