...
async function seedData() {
  const { rows: productRows } = await pool.query('SELECT COUNT(*) as count FROM products');
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (existingRows.length > 0) return res.status(400).json({ error: 'ეს ნომერი უკვე რეგისტრირებულია' });

    let referredBy = null;
    if (inviteCode) {
      const { rows: referrerRows } = await pool.query('SELECT id FROM users WHERE invite_code = $1', [inviteCode]);
      if (referrerRows.length === 0) return res.status(400).json({ error: 'მოწვევის კოდი არასწორია' });
...
app.post('/api/withdraw', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query('SELECT balance FROM users WHERE id = $1', [req.userId]);
    res.json({ success: true, balance: rows[0].balance, estimatedTime: '5-30 წუთი' });
  } catch (err) {
...
app.post('/api/blog', auth, async (req, res) => {
  try {
    ...
    const { rows: updatedRows } = await pool.query('SELECT balance FROM users WHERE id = $1', [req.userId]);
    res.json({ success: true, postId: insertedRows[0].id, reward, balance: updatedRows[0].balance });
  } catch (err) {
